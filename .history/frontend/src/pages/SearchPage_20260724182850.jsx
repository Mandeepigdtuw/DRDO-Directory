  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (isMinExperienceQuery(query)) {
        const response = await getAllPersonnelPublic('experience', 'asc')
        let all = [...response.data.results]
        const num = extractNumber(query)
        if (num !== null) all = all.filter(p => p.years_of_experience < num)
        data = all.map((p, i) => ({ ...p, relevance_score: Math.max(0.99 - i * 0.01, 0.1) }))
        ...
        data = [...response.data.results]
          ...
          const response = await getAllPersonnelPublic('experience', 'desc')
          data = response.data.results
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        <div className="flex flex-col items-center justify-center px-6 pt-24 pb-16">
          ...
          {/* Title */}
          <h1 className="text-6xl sm:text-7xl lg:text-8xl font-bold text-white text-center leading-tight mb-6">
            DRDO Personnel
          ...
          <div className="w-full max-w-3xl flex gap-3 mb-6">
            <div className="flex-1 flex items-center bg-slate-900 border-2 border-slate-700 hover:border-slate-500 focus-within:border-blue-500 focus-within:ring-4 focus-within:ring-blue-500/10 rounded-2xl px-6 gap-4 transition-all shadow-xl">
            ...
            >
              {loading ? <Loader2 size={20} className="animate-spin" /> : <Search size={18} />}
