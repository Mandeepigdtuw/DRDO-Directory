...
export default function AdminDashboard() {
  ...
  const stats = [
    {
      ...
      value: summary ? summary.total_personnel : '—',
      icon: Users,
      color: 'text-blue-400',
  ...
  const quickLinks = [
    { title: 'Manage Personnel', description: 'View, edit, or delete existing records', to: '/admin/personnel', icon: Users },
    { title: 'Add New Personnel', description: 'Add a new scientist or staff member', to: '/admin/personnel/add', icon: Plus },
    { title: 'Search Analytics', description: 'View search trends and top queries', to: '/admin/analytics', icon: BarChart2 },
  ]

  return (
    <AdminLayout maxWidth="max-w-5xl">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-white">Admin Dashboard</h1>
        <p className="text-slate-400 text-sm mt-1">Overview of the DRDO Personnel Directory system</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {quickLinks.map((link) => (
          <Link
            key={link.to}
            to={link.to}
            className="bg-slate-800/60 border border-slate-700 hover:border-blue-500/50 rounded-xl p-5 transition-all group block"
          >
