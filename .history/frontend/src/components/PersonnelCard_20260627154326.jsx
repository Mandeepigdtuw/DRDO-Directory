...
export default function PersonnelCard({ person, rank, onClick }) {
  ...
  const scoreColor =
    scorePercent >= 70 ? 'text-green-400 bg-green-400/10' :
    scorePercent >= 50 ? 'text-blue-400 bg-blue-400/10' :
    'text-slate-400 bg-slate-700/50'

  return (
    <div
      onClick={onClick}
      className="bg-slate-800/60 border border-slate-700 hover:border-blue-500/50 rounded-xl p-5 cursor-pointer transition-all hover:bg-slate-800 group"
    >
      {/* Header */}
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-blue-400 text-xs">{person.employee_id}</p>
          </div>
        </div>
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-slate-400 text-sm">{person.department}</span>
      </div>
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-slate-400 text-sm">
          {person.lab_acronym} · {person.years_of_experience} years experience
        </span>
      </div>
