...
def extract_keywords(query: str) -> list:
    """
    Extract potential exact-match keywords from a longer query.
    Looks for capitalized words, known project names, lab names.
    """
    words = query.split()
    keywords = []
    for word in words:
        clean = re.sub(r'[^a-zA-Z0-9]', '', word)
        # Capitalized words longer than 3 chars are likely proper nouns
        if len(clean) > 3 and clean[0].isupper():
            keywords.append(clean)
        # All caps words are likely acronyms or project codes
...
def hybrid_search(query: str, top_k: int = 10) -> list:
    """
    For long queries: run exact match on extracted keywords first,
    ...
    # Run exact match for each extracted keyword
    for keyword in keywords:
        matches = exact_match_search(keyword)
        if matches:
            scored = score_exact_results(matches, keyword)
            for person, score in scored:
                if person["employee_id"] not in exact_ids:
                    formatted = format_person(person)
                    formatted["relevance_score"] = round(min(score, 1.0), 4)
                    final_results.append(formatted)
                    exact_ids.add(formatted["employee_id"])

    # Fill remaining with semantic results
    remaining = top_k - len(final_results)
    if remaining > 0:
        semantic_fill = run_semantic_pipeline(query, top_k=top_k, exclude_ids=exact_ids)
